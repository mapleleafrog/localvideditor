...
| Footer | **Scenes strip** — a slide sorter: one card per scene (thumb · name · an inline **hold** seconds box · `+N` objects appearing), with an **arrow connector between cards holding the glide seconds** into the next scene and its speed dot (click the dot to apply the suggested duration). **▶ Preview all** plays the whole schedule in Live mode. **Click a card to select it** and jump the camera; drag to reorder. |
