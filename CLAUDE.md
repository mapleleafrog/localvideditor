...
- **Import footage:** drag-drop / file-pick → `POST /api/upload` writes into `public/media/` (served via Vite `publicDir=public`, so `staticFile("media/x")` resolves in both preview and render). Click a media tile to add a clip; shift-click to add an image overlay. Video clips get their `durationInFrames` auto-set from the file length (`getVideoMetadata`); images keep the default.
- **Arrange/edit:** clip track (with image/video **thumbnails**) — drag-reorder, select, delete; right-panel inspector for clip props (duration, motion, per-clip transition + length, video trim/volume) and overlay props (text/image fields, position/scale/opacity/depth, stacked effect chips); background editor. Overlays can be **moved / scaled / rotated directly on the preview** via gizmo handles (`Preview.tsx` maps pointer → x/y %, scale from center distance, rotation from center angle). **Undo/redo** via `editor/useHistory.ts` (toolbar ↶/↷ + Ctrl/Cmd+Z / Shift+Z; ignored while typing in a field); New/Open/Import reset the history.
...
- **Save/Load:** `💾 Save` → `POST /api/save` writes `projects/<name>.json` (validated as JSON server-side); `⬇ Download` uses the browser; `Open…` lists/loads `projects/*.json` via `/api/projects` + `/api/load`; `Import .json` loads an arbitrary file. All round-trips go through `src/timeline/projectIO.ts` (`serializeProject`/`parseProject` — schema-validated, never throws).
- **Render in-editor:** `🎬 Render MP4` → `POST /api/render` writes props to `out/<name>.props.json`, spawns the local `remotion render` binary, and **streams the CLI log back live**; on success it emits `__DONE__ <file>` and `GET /api/output?name=` serves `out/<file>.mp4` for download. The client parses the stream (`editor/renderLog.ts`) into a **progress bar + phase + friendly errors** (e.g. Chromium-unavailable), with the raw log behind a `<details>`. (Real renders need Chromium — blocked by a restrictive network egress policy in the cloud sandbox; works on a normal machine.)
- Editor files: `editor/{Editor,Preview,MediaLibrary,ClipTrack,Inspector,AudioPanel,ui}.tsx`, `editor/{model,options,api,useHistory,renderLog}.ts`, `editor/vite.config.mjs`. WebGL shader transitions are labeled `(shader*)` since their Player preview needs the Chrome flag (they still render).
