...
- **State:** zustand + zundo (`editor/src/store.ts`) — only `project` is undoable; a throttled `handleSet` (600ms, leading) coalesces rapid drags/scrubs/typing into ONE undo step (so Ctrl+Z reverts a whole motion, not a few px); autosaves to `localStorage`, seeded from `projects/sample.json`.
