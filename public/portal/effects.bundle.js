...
var SoranjiEffects = (() => {
  ...
  var MOTION_FORMULAS = {
    ...
    rgbSplitHeavy: ({ t }) => {
      ...
      return {
        filter: `drop-shadow(${o}px 0 0 rgba(255,0,90,0.75)) drop-shadow(${-o}px 0 0 rgba(0,225,255,0.75)) drop-shadow(0 ${o * 0.4}px 0 rgba(70,255,130,0.35))`
      };
    },
    ...
    romanticGlow: ({ t }) => ({
      filter: `drop-shadow(0 0 ${8 + Math.sin(t * 2) * 6}px rgba(255,120,170,0.85)) drop-shadow(0 0 ${18 + Math.sin(t * 2) * 10}px rgba(255,170,140,0.55)) brightness(1.05)`
    }),
    softFocusBreath: ({ t }) => ({
    ...
    sparkleGlow: ({ t, frame }) => {
      ...
      return {
        filter: `drop-shadow(${Math.sin(t * 2) * 6}px ${Math.cos(t * 2.3) * 6}px 0 rgba(255,255,255,${tw.toFixed(2)})) drop-shadow(${Math.cos(t * 1.7) * 8}px ${Math.sin(t * 2.1) * 8}px 1px rgba(255,240,180,${(tw * 0.7).toFixed(2)})) brightness(1.08)`
      };
    },
    // ======================= PACK: Wedding — full-frame atmospherics (FX overlay) =======================
    weddingPetals: ({ t }) => {
      const petal = (seed, xBase, size, speed, drift) => {
