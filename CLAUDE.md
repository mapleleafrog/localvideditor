...
- **In-app render + persistence** (`editor/render-plugin.ts`, a Vite dev-server plugin): `POST /api/render` (bundle once → `selectComposition` → `renderMedia`, streamed NDJSON progress) · `POST /api/save-project` (→ `projects/*.json`) · `POST /api/upload?name=&project=` (writes bytes to `public/media/<project>/`, suffixes on collision) · `POST /api/delete-project` (removes the JSON + media folder) · `GET /api/media?project=` (root `public/` + flat `public/media/` + that project's folder, split into `{ assets, audio }`). Same renderer/output as `npx remotion render Timeline --props=<json>`.
