...
- **Arrange/edit:** clip track with drag-reorder, select, delete; right-panel inspector for clip props (duration, motion, per-clip transition + length, video trim/volume) and overlay props (text/image fields, position/scale/opacity/depth, stacked effect chips); background editor. Overlays can also be **dragged to position directly on the preview** (handles in `Preview.tsx` map pointer → x/y %). **Undo/redo** via `editor/useHistory.ts` (toolbar ↶/↷ + Ctrl/Cmd+Z / Shift+Z; ignored while typing in a field); New/Open/Import reset the history.
...
- **Render in-editor:** `🎬 Render MP4` → `POST /api/render` writes props to `out/<name>.props.json`, spawns the local `remotion render` binary, and **streams the CLI log back live**; on success it emits `__DONE__ <file>` and `GET /api/output?name=` serves `out/<file>.mp4` for download. (Real renders need Chromium — blocked by a restrictive network egress policy.)
- Editor files: `editor/{Editor,Preview,MediaLibrary,ClipTrack,Inspector,AudioPanel,ui}.tsx`, `editor/{model,options,api,useHistory}.ts`, `editor/vite.config.mjs`. WebGL shader transitions are labeled `(shader*)` since their Player preview needs the Chrome flag (they still render).
