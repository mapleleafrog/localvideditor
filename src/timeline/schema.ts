...
const wallSceneSchema = z.object({
  ...
  easing: z.enum(["smooth", "gentle", "sine", "cubic", "settle"]).default("smooth"),
  ...
  /** HOVER — a deliberate slow drift DURING the hold (the camera never quite stops): `toward` =
   *  a slow creep in the direction the NEXT glide will travel (anticipation — the editor's default
   *  for new scenes), or a gentle push-in / pull-out / pan; eased with the same
   *  zero-velocity-at-both-ends curve as the glides, so it lands and departs seamlessly. The next
   *  glide starts from the drifted pose. Unset = a bit-exact still hold. Distinct from
   *  `breathing` (random handheld tremor). */
  hover: z.enum(["none", "toward", "pushIn", "pullOut", "left", "right", "up", "down"]).optional(),
...
export const wallSchema = z.object({
  ...
  /** FLOW landing, signed (−0.6 … 0.6, default 0.35).
   *  Positive = the fraction of each glide's TIME spent decelerating from the approach pace into
   *  the drift speed before the scene point (the fast part ends early).
   *  Negative = a LEAD: the glide lands that fraction of the hop SHORT of the scene point and the
   *  hold's slow drift carries the camera the rest of the way through it — the fast part covers
   *  less distance in the same time, so it is slower, and the scene point is passed mid-drift.
   *  0 = the drift speed is only met at the last frame, right on the scene point. */
  flowLand: z.number().min(-0.6).max(0.6).optional(),
