...
## Audio (soundtrack)
`projectSchema.audio` is an array of tracks (`src`, `volume`, `from`, `trimBefore`, `trimAfter`, `loop`); `Timeline.tsx`'s `SoundtrackLayer` renders each as `<Audio>` inside a `<Sequence from={track.from}>` so music plays under the whole video (preview + render). Volume uses the callback form (`volume={() => a.volume}`) per Remotion's lint rule. NOTE: audio does NOT extend the timeline — duration is still clip-driven, so size your clips to the song. Beat-reactive motions are still on the synthetic 120 BPM clock (`beatKick`); real audio-reactive beat-sync (`@remotion/media-utils` `useAudioData`/`visualizeAudio`) remains Stage 3.
